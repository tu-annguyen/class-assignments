...
public:
   ...
   // pre: position()<size()
   LIST_ELEMENT peekNext();

   // peekPrev()
   // Returns the element before the cursor.
   // pre: position()>0
   LIST_ELEMENT peekPrev();  

   // moveNext()
   // Advances cursor to next higher position. Returns the List element that
   // was passed over. 
   // pre: position()<size() 
   LIST_ELEMENT moveNext();

   // movePrev()
   // Advances cursor to next lower position. Returns the List element that
   // was passed over. 
   // pre: position()>0
   LIST_ELEMENT movePrev();

   // insertAfter()
   // Inserts x after cursor.
   void insertAfter(LIST_ELEMENT x);

   // insertBefore()
   // Inserts x before cursor.
   void insertBefore(LIST_ELEMENT x);
